...
def get_emission_prob(word, tag, count_dict):
    if word in all_words:
        return emission_prob(word, tag, count_dict)
    else:
        return emission_prob("_RARE_", tag, count_dict)
...
def pi(k, tag1, tag2, dp_dict):
    if (k, tag1, tag2) in dp_dict:
        ...
        # print "pi of ", k, tag1, tag2, max_prob
        return max_prob
...
if __name__ == "__main__":
    ...
    with open("gene.dev", "r") as dev:
        with open("gene_dev.p2.out", "w") as output:
